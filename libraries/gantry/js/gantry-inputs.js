...
var InputsMorph = {
    ...
    morph: function(e, d) {
        ...
        if (h && h.get("tag") == "label") {
            ...
            if (InputsMorph.rtl && Browser.Engine.gecko) {
                e.setStyles({
                    position: "absolute",
                    right: "-10000px"
                });
            ...
            if (InputsMorph.rtl && (Browser.Engine.presto || Browser.Engine.trident)) {
    ...
    addEvent: function(e, d) {
        ...
        if (Browser.Engine.presto || Browser.Engine.trident || (e.getNext() && !e.getNext().getProperty("for"))) {
            var g = e.getNext(),
                f = e.getParent();
            if (g && g.get("tag") == "label" && (Browser.Engine.trident || (Browser.Engine.presto && !e.opera))) {
                g.addEvent("click", function() {
                    if ((Browser.Engine.presto || Browser.Engine.trident) && !e.opera) {
                        e.opera = true;
                    }
                    e.fireEvent("click");
                });
